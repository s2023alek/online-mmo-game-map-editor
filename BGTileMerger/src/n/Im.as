package n {
	...
	public class Im extends Loader {
		 ...
		 */
		function Im(path_:String=null, name_:String=null, init:Function=null/*init first*/, complete:Function=null, err:Function=null, progress:Function=null) {
			//trace("LOAD path_ = " + path_);
		...
		private function onLoadingProgress(e:ProgressEvent):void {if (onProgress!=null) {onProgress(this, e);}}
		...
		private function er(e:Event):void {
			//tr("Im error loading: " + e.toString());
		...
		private static function textureLoaded():void {
			tr('\ntextureLoaded()');
			if (lb_current) {tr('lb_current');return;}
		...
		private static function loadNextTexture():void {
			...
			loader_.addEventListener(Event.COMPLETE, l_complete, false, 0, true);
			loader_.addEventListener(IOErrorEvent.IO_ERROR, l_err, false, 0, true);
			loader_.addEventListener(SecurityErrorEvent.SECURITY_ERROR, l_err, false, 0, true);
