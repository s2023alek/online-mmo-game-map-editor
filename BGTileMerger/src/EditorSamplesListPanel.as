package {
	 ...
	 */
	public class EditorSamplesListPanel {
		
		//{ =^_^= CONSTRUCTOR
		
		private var testMode:Boolean=Boolean(0);
		
		function EditorSamplesListPanel (w:uint, h:uint) {
			this.w=w;
			this.h=h;
		...
		public function setContent(title:Vector.<String>, title0:Vector.<String>, image:Vector.<DisplayObject>, id:Vector.<uint>, cols:uint):void {
			if (scrollPane) {
			...
			for (var i:uint in title) {
				...
				item = new ImageListElement(title[i], title0[i], image[i], id[i], buttonPressed, image[i].width, image[i].height);
...
class ImageListElement extends JPanel {
	 ...
	 * @param	image
	 * @param	id_
	 * @param	buttonPressedRef (id, bSN):void;
	 */
	function ImageListElement (title:String, title0:String, image:DisplayObject, id_:uint, buttonPressedRef:Function, imW:int=-1, imH:int=-1) {
		...
		selectButton = new JButton("", new AssetIcon(image, imW, imH));
		selectButton.addActionListener(buttonPressed);
		append(selectButton, BorderLayout.CENTER);
		
		selectButton2 = new JButton(title0);
	...
	private var selectButton2:JButton;
	private var id:uint;
